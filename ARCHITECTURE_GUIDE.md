...
> ⚠️ **Номери версій — глобальні для всієї БД, а не для модуля.** Кожен модуль
> оголошує свою `db.version(N)`, але всі вони працюють над одним екземпляром
> `Dexie`. Тому номери не повинні конфліктувати між модулями, і нова версія, що
> змінює лише «свою» таблицю, успадковує решту схеми від найближчої нижчої версії
> — яку міг оголосити **інший модуль**. Dexie сортує версії за номером при
> `.open()`, тож порядок імпорту `*db.js` не впливає на фінальну схему **за
> однієї умови**: усі модулі зареєстрували версії до першого звернення до БД.
> Звідси правило: **не роби eager-запитів до `db` на рівні модуля** (тобто при
> імпорті файлу) — лише в `onMounted`/Service-викликах. Інакше БД відкриється з
> неповною схемою, бо частину версій ще не зареєстровано.
>
> _Приклад із цього проєкту:_ базовий `db/index.js` оголошує v14 (індекс `ipn`),
> а `modules/documents/db.js` — v13 (таблиця `documentTemplates`). База
> завантажується раніше, але v14 коректно успадковує `documentTemplates`, бо
> перший доступ до БД відбувається вже після реєстрації всіх версій.
